...
package dev.teogor.paletteon.dynamiccolor
...
public class DynamicColor(
  ...
  public fun getTone(scheme: DynamicScheme): Double {
    ...
    return if (toneDeltaPair != null) {
      ...
      var nTone = if (dev.teogor.paletteon.contrast.Contrast.ratioOfTones(
          bgTone,
          nInitialTone
        ) >= nContrast
      ...
      var fTone = if (dev.teogor.paletteon.contrast.Contrast.ratioOfTones(
          bgTone,
          fInitialTone
        ) >= fContrast
      ...
      if (dev.teogor.paletteon.contrast.Contrast.ratioOfTones(bgTone, answer) >= desiredRatio) {
      ...
      if (isBackground && 50 <= answer && answer < 60) {
        // Must adjust
        answer = (if (dev.teogor.paletteon.contrast.Contrast.ratioOfTones(
            49.0,
            bgTone
          ) >= desiredRatio
        ) 49.0 else 60.0)
      ...
      if (secondBackground != null) {
        ...
        if (dev.teogor.paletteon.contrast.Contrast.ratioOfTones(upper, answer) >= desiredRatio
          && dev.teogor.paletteon.contrast.Contrast.ratioOfTones(lower, answer) >= desiredRatio
        ) {
          return answer
        }
        ...
        val lightOption: Double =
          dev.teogor.paletteon.contrast.Contrast.lighter(upper, desiredRatio)

        // The lightest dark tone that satisfies the desired ratio,
        // or -1 if such ratio cannot be reached.
        val darkOption: Double = dev.teogor.paletteon.contrast.Contrast.darker(lower, desiredRatio)
  ...
  public companion object {
    ...
    public fun foregroundTone(bgTone: Double, ratio: Double): Double {
      val lighterTone: Double = dev.teogor.paletteon.contrast.Contrast.lighterUnsafe(bgTone, ratio)
      val darkerTone: Double = dev.teogor.paletteon.contrast.Contrast.darkerUnsafe(bgTone, ratio)
      val lighterRatio: Double =
        dev.teogor.paletteon.contrast.Contrast.ratioOfTones(lighterTone, bgTone)
      val darkerRatio: Double =
        dev.teogor.paletteon.contrast.Contrast.ratioOfTones(darkerTone, bgTone)
