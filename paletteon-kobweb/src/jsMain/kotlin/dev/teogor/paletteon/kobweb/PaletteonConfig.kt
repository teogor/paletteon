...
import dev.teogor.paletteon.kobweb.internal.getColorModeFromLocalStorage
...
public interface PaletteonConfig {
   ...
   *
   * @return The [PaletteonTheme] representing the initial theme configuration.
   */
...
public class MutablePaletteonConfig : PaletteonConfig {
   ...
   * to be updated dynamically. By default, it is initialized with an empty [PaletteonTheme].
   */
 ...
 * as needed.
 */
internal var MutablePaletteonConfigInstance: MutablePaletteonConfig = MutablePaletteonConfig()

/**
 * Initializes the Paletteon configuration based on values stored in local storage.
 *
 * This function reads the initial configuration settings for Paletteon from local storage
 * and applies them to the [PaletteonConfig] instance. It also sets the initial color mode
 * from local storage or defaults to [ColorMode.DARK] if not found.
 *
 * @param ctx The [InitSilkContext] used to initialize the Paletteon configuration.
 */
public fun initPaletteon(ctx: InitSilkContext) {
  ctx.config.initialColorMode = getColorModeFromLocalStorage()

  (PaletteonConfig.Instance as MutablePaletteonConfig).let {
    it.initialPaletteonTheme = getInitialPaletteonTheme()
  }
}
